...
### Current Task: re-test FQ-ALGO-9 fix; Continue-flag question still open (2026-06-20)

`CODEX_PROMPT_15` came back with a confirmed root cause (live-reproduced 5/5 cycles, both repos): `Event.prototype.stopImmediatePropagation` is overridden (~line 5876) to silently no-op for Enter/Space while a reveal is open. A handler at ~line 5662 correctly detects reveal-open, calls `advance()`, then tries to stop propagation — the shim neuters that call, so the base bubble-phase handler (~line 432, fires last) still runs for the SAME event, now sees the *new* card's closed reveal, and fires `selectSolo(selected)` on it.

**Fixed 2026-06-20 (PHASE2 `a5906a7` / PHASE1 `4ea0a13`):** did not touch the shim (unknown blast radius across 20+ listeners, no comment explaining its other conditions). Instead: `wrappedAdvance` now stamps a timestamp at the single point every `advance()` call funnels through; line 432's ambiguous branch skips `selectSolo` if an advance happened within the last 50ms. Verified via JXA simulation. **Not live-browser-validated** — re-run PROMPT_15's exact reproduction against this fix before considering FQ-ALGO-9 fully closed.

**Still unresolved, separate from FQ-ALGO-9:** the Continue-button flag question from FQ-ALGO-8. Codex's first redesigned test was itself flawed (the forced-wrong selection had already committed 'again' via select-time rating before Continue was even clicked, so the 'again' result it saw doesn't prove anything about the flag). Needs the actual fix to that test: clear `__cozyPendingRating20260603` and `__cozyLastRatedId` AFTER the wrong reveal settles, THEN click literal Continue, THEN check the result.
