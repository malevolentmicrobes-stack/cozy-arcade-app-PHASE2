...
**Deploy confirmed unblocked.** The 2026-06-23 git-auth block (see AGENTS.md "Prior Task") resolved the same evening — two more commits (`43f6a93` runner-pointer-events fix, `2ff95d2` doc close) landed and pushed cleanly after that entry was written. Verified directly via `git fetch` + log, not by trusting the doc: PHASE2 `main`==`public` at `cozy-arcade-PHASE2-v49`, PHASE1 `main` at `cozy-arcade-v85`, both working trees clean (aside from one unrelated uncommitted PHASE1 doc note, see below). Lesson: this project's session-log docs get appended to inconsistently under time pressure — always re-verify deploy/auth state with git directly rather than trusting whichever doc was last touched.

**Full Codex browser retest against `main` 2ff95d2/v49** (real Upload→Solo→reveal→Space advance, iPhone-sized viewport, Domain mode), explicitly superseding the older v46/`fee324a` handoff. Confirmed fixed/live: homeTopBtn CSS (independently reconfirmed), stale-visible-dx-after-Space, runner tap interception (390x844 and 375x667). Did not reproduce this round (normal card, one flow): duplicate board-trigger rendering, the "Gate Completed"→"Learning Moment" title flip — flagged as "didn't reproduce" not "fixed for good," since the 06-22 findings used a board-trigger-only card with longer dwell. Gates held: FSRS 17/17, smoke 6/6, no page errors.

**REVEAL-TRIGGER-CHURN remains the top open item** — 89 mutations measured for one normal reveal, reveal rendering is still multi-writer with no single idempotent owner. New finding: hidden reveal content still mutates to the next card's text after Space while the panel is hidden, proving the reveal chain isn't fully inert post-advance even though the visible symptom is fixed. Methodology correction: `window.current` was observed null/absent (not merely disagreeing) during this retest — it's not a reliable validation signal; future diagnostics should bind to rendered DOM + fixture IDs instead. `CODEX_PROMPT_17` (reveal ownership instrumentation) was updated with this session's exact element list (`#soloRevealDx`, `.oneThing350`, `#soloRevealRatings` added to the injected snapshot) and is queued, ready to send — do not add another reveal wrapper.

**New, separate finding:** Domain orb's normal click is unstable under browser automation (3s timeout, forced click works and renders correctly) — logged as a possible touch-target/moving-target UX risk in `OPEN_DIFFERENTIALS.md` `DOMAIN-TIMER`, not yet confirmed user-facing on a real device, not yet actioned.

**Found, not yet committed (PHASE1 only):** `MOBILE_RECTIFIER_PLAN.md` has an uncommitted edit logging `COZY-HOME-DUPE-MOBILE` (user iPhone screenshot showing two stacked home buttons during PHASE1 Solo gameplay) — distinct from the already-fixed PHASE2 homeTopBtn bug. Sitting dirty as of this session; flagged for the user to confirm before committing.

**Deploy infra changed:** PHASE2's GitHub Pages source switched from `public` branch to `main` (the split had no privacy/curation purpose — both repos are public on GitHub regardless, and `public` was just a lagging mirror) — confirmed live at v49, retires the `git push origin main:public --force` step from the deploy workflow. **PHASE1's Pages deploy is still stuck on `v65`** (local `main` is `v85`) despite `.nojekyll` and one same-value Settings re-save; needs a genuine source-value toggle (e.g. to "None" and back) to kick the build loose, not yet tried.

**User reviewed the retest writeup and gave explicit sign-off, plus one new finding.** Accepted/closed, don't reopen without new evidence: runner click interception, PHASE2 homeTopBtn, Space-advance stale-visible-reveal. Confirmed REVEAL-TRIGGER-CHURN is correctly the top-priority real bug. **New, separately tracked (explicitly not bundled under the closed homeTopBtn item):** `HUD-ICON-BLANK` (in-game HUD icon buttons render blank outside one narrow breakpoint) / `OLD-SETTINGS-LEAK` (legacy `#gearBtn` can coexist with the newer in-HUD settings proxy). Claude source-verified every claimed line number in both repos before logging anything: `.hudIconButton371::before` (PHASE2 index.html:12500, PHASE1:12486) is the only icon-glyph-paint rule in either file and it's confined inside `@media (max-width:900px)...` with no unconditional fallback — confirmed root cause, more precise than the original "doesn't apply at this breakpoint" framing. The `#gearBtn` in-game hide rule (PHASE2:12506-12508, PHASE1:12492) is gated inside that exact same media query, explaining the coexistence. `window.undoReview` (PHASE2:13403, PHASE1:13300) confirmed unrelated. Full detail and recommended fix (two independent changes — move the icon rule outside the media gate; pick one settings owner, a product-intent choice) logged in `OPEN_DIFFERENTIALS.md`. Not yet fixed — analysis only, no code changed this session.

**Correction, same day, caught by Codex's independent cross-repo browser probe before the docs commit above got pushed:** Claude's "both findings hold identically in PHASE1" claim was wrong — PHASE2's media gate is an OR (`max-width:900px` OR short-landscape); PHASE1's is `max-width:900px` only, no landscape clause. Re-verified directly via `sed` before accepting the correction, not taken on faith. Net effect: PHASE1 stays broken at short-landscape where PHASE2's OR-clause would have saved it. Two new subfindings from the same probe: PHASE2's HUD settings button renders partially offscreen on iPhone viewport (browser-measured, root cause not yet pinned); PHASE1 has a PHASE2-absent `vHUD-polish-2` block that unconditionally hides its in-HUD home/exit button at every breakpoint, plausibly related to `COZY-HOME-DUPE-MOBILE`. REVEAL-TRIGGER-CHURN mutation counts re-measured much higher (493/620 PHASE2, 457/589 PHASE1) under a real-uploaded-deck/iPhone-viewport probe than today's earlier desktop figure (89) — different conditions, not a contradiction, both logged. Lesson written into `AGENTS.md`: matching line numbers across repos is not proof of matching behavior. No code changed.
