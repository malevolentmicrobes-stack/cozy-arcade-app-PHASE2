...
4. Then audit `selected`/runner lane reset: every new card must set `selected = 0` or an explicit neutral lane, update visuals, and never derive selection from the correct answer.
5. Then reconcile progress state with a single `phase3State.progress` write path and keep legacy `state` as a mirror only.
